""" residual_network_bn.py
    Residual model architecture with BatchNorm.
    November 2020
"""
...
class ResidualNetworkBN(nn.Module):
    """Modified ResidualNetwork_bn model class"""

    def __init__(self, block, num_blocks, num_outputs, width, depth, in_channels, pool_size):
        super(ResidualNetworkBN, self).__init__()
...
def residual_network_bn(num_outputs, depth, width, dataset):
    in_channels = {"CIFAR10": 3, "SVHN": 3, "EMNIST": 1}[dataset.upper()]
    pool_size = {"CIFAR10": 8, "SVHN": 8, "EMNIST": 7}[dataset.upper()]
    return ResidualNetworkBN(BasicBlock, [2], num_outputs, width, depth, in_channels, pool_size)
